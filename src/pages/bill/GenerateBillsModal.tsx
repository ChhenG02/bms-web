...
// Mock clients data
const DEFAULT_CLIENTS = [
  { id: "C-001", name: "Sok Sopha", room: "A01" },
  { id: "C-002", name: "Vong Chantha", room: "A02" },
  { id: "C-003", name: "Heng Sovanna", room: "B01" },
  { id: "C-004", name: "Mao Chanborey", room: "B02" },
  { id: "C-005", name: "Touch Rana", room: "C01" },
  { id: "C-006", name: "Khun Leena", room: "C02" },
  { id: "C-007", name: "Noun Sotheara", room: "D01" },
  { id: "C-008", name: "Sin Vichetr", room: "D02" },
];
...
export function GenerateBillsModal({
  ...
  const steps = [
    { title: "Bill Type", icon: <CalendarOutlined /> },
    { title: "Select Customers", icon: <TeamOutlined /> },
    { title: "Set Prices", icon: <DollarOutlined /> },
    { title: "Confirm & Create", icon: <CheckOutlined /> },
  ];

  // Preview data (would come from API in real implementation)
  const previewData = {
    period: billingType === "monthly" 
      ? (form.getFieldValue("month") || "May 2025")
      : "Custom Date Range",
    tenantsSelected: tenantScope === "all" 
      ? clients.length 
      : (form.getFieldValue("selectedClients")?.length || 0),
    estimatedBills: tenantScope === "all" 
      ? clients.length 
      : (form.getFieldValue("selectedClients")?.length || 0),
    dueDate: `Day ${form.getFieldValue("dueDay") || 25} of next month`,
    electricityRate: `${(form.getFieldValue("electricityRate") || 900).toLocaleString()}៛/kWh`,
    estimatedTotal: `${((form.getFieldValue("electricityRate") || 900) * 150 * (tenantScope === "all" ? clients.length : (form.getFieldValue("selectedClients")?.length || 0))).toLocaleString()}៛`,
  };
      ...
      >
        ...
        {currentStep === 0 && (
          <div>
            <div style={{ marginBottom: 20 }}>
              ...
              >
                <Space direction="vertical">
                  ...
                  <Radio value="custom">
                    <span style={{ fontWeight: 500 }}>Custom Date Range</span>
                    <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>
              ...
              {billingType === "monthly" ? (
                <Form.Item name="month" rules={[{ required: true, message: "Please select a month" }]}>
                  <Select
            ...
            <Alert
              message="Information"
              description="The system will automatically calculate based on recorded meter data"
              type="info"
              showIcon
              style={{ borderRadius: 10, background: "#eff3fe", border: "none" }}
            />
        ...
        {currentStep === 1 && (
          <div>
            ...
            >
              <Space direction="vertical">
                <Radio value="all">
                  <span style={{ fontWeight: 500 }}>All Customers</span>
                  <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>
                    Generate bills for all active customers
                  </div>
                </Radio>
                <Radio value="selected">
                  <span style={{ fontWeight: 500 }}>Select Customers</span>
                  <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>
            ...
            {tenantScope === "selected" && (
              <Form.Item name="selectedClients" rules={[{ required: true, message: "Please select customers" }]}>
                <Select
                  mode="multiple"
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: "#6b7280", fontSize: 13 }}>Number of customers selected</span>
                <span style={{ fontWeight: 700, fontSize: 18, color: "#4f74e8" }}>
                  {tenantScope === "all" ? clients.length : (form.getFieldValue("selectedClients")?.length || 0)}
                </span>
              </div>
        ...
        {currentStep === 2 && (
          <div>
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item label="Electricity Rate" name="electricityRate">
                  <InputNumber<number>
                    min={0}
                    step={50}
                    formatter={(v) => `${v}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item label="Due Date" name="dueDay">
                  <InputNumber
                    min={1}
                    max={31}
                    style={{ width: "100%", borderRadius: 10 }}
              ...
              <Col span={12}>
                {/* <Form.Item label="Late Fee (%/month)" name="lateFeePercent">
                  <InputNumber
                    min={0}
                    max={100}
                    step={0.5}
            ...
            >
              <div style={{ fontSize: 12, color: "#166534", display: "flex", alignItems: "center", gap: 8 }}>
                <CheckOutlined style={{ color: "#16a34a" }} />
                These prices will be applied to all generated bills
              </div>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            >
              <Row gutter={[16, 16]}>
                <Col span={12}>
                  <Statistic
                    title={<span style={{ fontSize: 12, color: "#6b7280" }}>Billing Period</span>}
                    value={previewData.period}
                    valueStyle={{ fontSize: 14, fontWeight: 600, color: "#374151" }}
                  />
                </Col>
                <Col span={12}>
                  <Statistic
                    title={<span style={{ fontSize: 12, color: "#6b7280" }}>Number of Customers</span>}
                    value={previewData.tenantsSelected}
                    valueStyle={{ fontSize: 14, fontWeight: 600, color: "#4f74e8" }}
                  />
                </Col>
                <Col span={12}>
                  <Statistic
                    title={<span style={{ fontSize: 12, color: "#6b7280" }}>Estimated Bills</span>}
                    value={previewData.estimatedBills}
                    valueStyle={{ fontSize: 14, fontWeight: 600, color: "#374151" }}
                  />
                </Col>
                <Col span={12}>
                  <Statistic
                    title={<span style={{ fontSize: 12, color: "#6b7280" }}>Due Date</span>}
                    value={previewData.dueDate}
                    valueStyle={{ fontSize: 14, fontWeight: 600, color: "#d97706" }}
                  />
                </Col>
            ...
            >
              <div>
                <div style={{ fontSize: 12, color: "#6b7280" }}>Electricity Rate</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: "#4f74e8" }}>{previewData.electricityRate}</div>
              </div>
              <Divider type="vertical" style={{ height: 40 }} />
              <div>
            ...
            <Alert
              message="Please Confirm Information"
              description="Once bills are generated, you cannot modify the prices"
              type="warning"
              showIcon
              style={{ marginTop: 16, borderRadius: 10 }}
            />
        ...
        <div style={{ marginTop: 28, display: "flex", justifyContent: "space-between" }}>
          ...
          >
            {currentStep === steps.length - 1 ? (
              <span><CheckOutlined /> Generate Bills</span>
            ) : (
              <span>Next <ClockCircleOutlined /></span>
            )}
          </Button>
        </div>
      </Form>
